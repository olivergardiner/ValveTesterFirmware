...
void infoCommand(int index) {
  ...
  switch (index) {
    case 0: // H/W Version info
      Serial.println("OK: Info(0) = Rev 4 (Mega Pro)");
      break;
...
void setCommand(int index, int intParam) {
  ...
  switch (index) {
    ...
    case VG1: // Grid 1 voltage
      if (intParam < 0 || intParam+vg1Offset > 4095) {
      ...
      setGridVolts();
      break;
    case VG2: // Grid 1 voltage
      if (intParam < 0 || intParam + vg2Offset > 4095) {
        success = -ERR_GRID_RANGE;
      } else {
        targetValues[index] = intParam;
      }
    ...
    case SET_AVERAGE_MODE:
      ...
      break;
    case SET_VG1_OFFSET:
      if (intParam < 0 || intParam > 256) {
        success = -ERR_OFFSET_RANGE;
      } else {
        vg1Offset = intParam;
      }
      setGridVolts();
...
void setGridVolts() {
  ...
  int vg1 = targetValues[VG1] + vg1Offset;
  if (vg1 > 4095) {
    vg1 = 4095;
  }

  int vg2 = targetValues[VG2] + vg2Offset;
  if (vg2 > 4095) {
    vg2 = 4095;
  }
